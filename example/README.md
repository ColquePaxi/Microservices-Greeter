...
Padrão monolítico (para efeito de desenvolvimento): 
  ...
  - [x] Criar um novo serviço (helper service) que vai pegar um evento emitido pelo serviço greeter.hello (branch: feature/helper)
  - [x] Registrar o novo serviço (branch: feature/helper)
    mol$ load services/helper.service.js
    Tem que retornar:
    Loaded successfully!
    Service 'helper' registred.
  - [x] Emitir um evento no greeter.hello para ver se o helper service está escutando e retornando (branch: feature/helper)
    mol$ emit "hello.called" --payload "Acionei lá do Terminal"
    Tem que retornar:
    Helper service pegou um evento
    { payload: 'Acionei lá do Terminal' }
